...
public class ReadyMadeATMContext(DbContextOptions<ReadyMadeATMContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Transaction>().HasData(
            new Transaction
            {
                Id = 1, Type = "Deposit", Amount = 500, Timestamp = DateTime.Now.AddMonths(-2), CurrentBalance = 1500,
                SenderId = 1
            },
            new Transaction
            {
                Id = 2, Type = "Withdraw", Amount = 200, Timestamp = DateTime.Now.AddMonths(-1), CurrentBalance = 1300,
                SenderId = 1
            },
            new Transaction
            {
                Id = 3, Type = "Deposit", Amount = 1000, Timestamp = DateTime.Now.AddMonths(-1), CurrentBalance = 3000,
                SenderId = 2
            },
            new Transaction
            {
                Id = 4, Type = "Withdraw", Amount = 500, Timestamp = DateTime.Now.AddMonths(-2), CurrentBalance = 2500,
                SenderId = 2
            }
        );
